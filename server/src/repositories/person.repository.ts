...
export class PersonRepository {
  ...
  async createFaceCluster(faceCluster: Insertable<FaceClusterTable>) {
    return Object.values(faceCluster).some((value) => value !== undefined)
      ? this.db.insertInto('face_cluster').values(faceCluster).returning('id').executeTakeFirstOrThrow()
      : this.db.insertInto('face_cluster').defaultValues().returning('id').executeTakeFirstOrThrow();
  }
  ...
  async updateFaceCluster({ id, ...faceCluster }: Updateable<FaceClusterTable> & { id: string }) {
    return this.db
      .updateTable('face_cluster')
      .set(faceCluster)
      .where('face_cluster.id', '=', id)
      .returningAll()
      .executeTakeFirstOrThrow();
  }
  ...
  @GenerateSql({ params: [DummyValue.UUID, DummyValue.UUID] })
  getPersonForFaceCluster(faceClusterId: string, ownerId: string) {
    return this.db
      .selectFrom('person')
      .selectAll()
